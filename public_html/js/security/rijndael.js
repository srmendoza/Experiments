...
private function f4(x)
{
	return ((x<<2) ^ (((x>>6) & 1) * WPOLY) ^ (((x>>6) & 2) * WPOLY));
}

private function f8(x)
{
	return ((x<<3) ^ (((x>>5) & 1) * WPOLY) ^ (((x>>5) & 2) * WPOLY) ^ (((x>>5) & 4) * WPOLY));
}

private function d2(x)
{
	return (((x) >> 1) ^ ((x) & 1 ? DPOLY : 0));
}

private function f3(x)
{
	return (f2(x) ^ x);
}

private function f9(x)
{
	return (f8(x) ^ x);
}

private function fb(x)
{
	return (f8(x) ^ f2(x) ^ x);
}

private function fd(x)
{
	return (f8(x) ^ f4(x) ^ x);
}

private function fe(x)
{
	return (f8(x) ^ f4(x) ^ f2(x));
}
...
private function block16_copy( d, s )
{
	$d[0] = $s[ 0];
	$d[ 1] = $s[ 1];
	$d[ 2] = $s[ 2];
...
/*
private function copy_block( void * d, void *s, $nn )
{
	while( $nn-- )//revisar
	{
		*((uint_8t*)d)++ = *((uint_8t*)s)++;
...
private function xor_block( d, s )
{
	$d[ 0] ^= $s[ 0];
	$d[ 1] ^= $s[ 1];
	$d[ 2] ^= $s[ 2];
...
private function copy_and_key( d, s, k )
{
	$d[ 0] = $s[ 0] ^ $k[ 0];
	$d[ 1] = $s[ 1] ^ $k[ 1];
	$d[ 2] = $s[ 2] ^ $k[ 2];
...
private function add_round_key( d, $k )
{
	xor_block($d, $k);
return d;
}

private function shift_sub_rows( st)
{
	var tt;
	st[ 0] = s_box[st[ 0]];
	st[ 4] = s_box[st[ 4]];
...
private function inv_shift_sub_rows( st)
{
	var tt;
...
private function mix_sub_columns( dt )
{
	var st=[];
	block16_copy(st, dt);
...
private function inv_mix_sub_columns( dt )
{
	var st=[];
	block16_copy($st, $dt);
...
public function aesset_key( &$key, $keylen, &$ctx )
{
	var cc;
	var rc;
	var hi;
	...
	copy_block($ctx->ksch, $key, $keylen);//revisar
	hi = ($keylen + 28) << 2;
	$ctx->rnd = ($hi >> 4) - 1;
	for(cc = $keylen, rc = 1; cc < hi; cc += 4 )
	{
		...
		var t0 = $ctx->ksch[$cc - 4];
		var t1 = $ctx->ksch[$cc - 3];
		var t2 = $ctx->ksch[$cc - 2];
		var t3 = $ctx->ksch[$cc - 1];
		if(cc % $keylen == 0 )
		{
			$tt = $t0;
			t0 = s_box[$t1] ^ $rc;
...
public function aesencrypt( &$in, &$out, &$ctx )
{
	var s1=[];
	var r;
...
public function aesdecrypt( &$in, &$out, &$ctx )
{
	var s1=[];
	var r;
	...
	for(r = $ctx->rnd ; r>0 ; r-- )//for( $r = $ctx->rnd ; --$r ; )//revisar 
	{
		add_round_key( $s1, $ctx->ksch + r * N_BLOCK );
...
public function update_encrypt_key( &$k, &$rc )
{
	$k[0] ^= s_box[$k[29]] ^ $rc;
	$k[1] ^= s_box[$k[30]];
	$k[2] ^= s_box[$k[31]];
...
public function aes_encrypt( &$in, &$out, &$key, $o_key )
{
	var s1=[];
	var rc = 1;
	if($o_key != $key)
...
public function update_decrypt_key( &$k, &$rc )
{
	var cc;
...
256 bit keying
*/
public function aes_decrypt( &$in, &$out, &$key, $o_key)
{
	var s1=[];
	var r;
	var rc = 0x80;
	...
	for( r = 14 ; r>0 ; r--)//for( $r = 14 ; --$r ; )//revisar 
	{
		if(( r & 1 ))
		{
